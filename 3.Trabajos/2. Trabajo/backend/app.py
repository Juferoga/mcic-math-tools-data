...
app = FastAPI(title="M/M/k/k Simulation API", version="0.1")
...
class SimRequest(BaseModel):
    ...
    k: int = Field(
        1, gt=0, description="Número de servidores (k) — sin cola, modelo M/M/k/k"
    )
    sample_size: int = Field(10000, gt=0, description="Número de arribos a simular")
    seed: Optional[int] = Field(
        None, description="Semilla opcional para reproducibilidad"
    )
...
async def simulate(req: SimRequest):
    ...
    try:
        ...
        res = simulate_from_rates(
            lam=req.lam, mu=req.mu, k=req.k, sample_size=req.sample_size, seed=req.seed
        )
    ...
    raw_waits = (
        res.wait_times.tolist()
        if hasattr(res.wait_times, "tolist")
        else list(res.wait_times)
    )
    waits = [
        float(w) for w in raw_waits if not (isinstance(w, float) and math.isnan(w))
    ]
