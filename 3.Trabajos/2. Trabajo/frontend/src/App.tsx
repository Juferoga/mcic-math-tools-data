import React from 'react'
import Sidebar from './components/Sidebar'
import SensitivityChart from './components/SensitivityChart'
import StatsPanel from './components/StatsPanel'
import APMPanel from './components/APMPanel'
import ExportPanel from './components/ExportPanel'
import Spinner from './components/Spinner'
import { useSimulation } from './hooks/useSimulation'

const App: React.FC = () => {
  const { loading, error, result, sensitivity, runSimulation, runSensitivity } = useSimulation()

  const handleSimulate = async (params: any) => {
    try {
      await runSimulation(params)
    } catch (e) {
      // handled in hook
    }
  }

  const handleSensitivity = async (opts: any) => {
    try {
      await runSensitivity(opts)
    } catch (e) {
      // handled in hook
    }
  }

  const avgConcurrent = result?.states && result.states.length > 0 ? result.states.reduce((a, b) => a + b, 0) / result.states.length : undefined

  return (
    <div className="min-h-screen flex relative">
      {/* floating background accents */}
      <div className="floating-accent accent-aurora" aria-hidden />
      <div className="floating-accent accent-sunset" aria-hidden />

      <aside className="w-80 p-4 border-r border-transparent bg-transparent">
        <div className="glass-card glass-border-gradient">
          <h1 className="text-xl font-bold mb-4 text-white">MM1K — Dashboard</h1>
          <Sidebar onSimulate={handleSimulate} onSensitivity={handleSensitivity} loading={loading} />
          <ExportPanel />
          {error && <div className="mt-4 text-sm text-red-400">Error: {error}</div>}
        </div>
      </aside>

      <main className="flex-1 p-6 relative">
        {/* export area wraps the APM and charts to capture them together */}
        <div id="export-area">
          <div className="glass-card mb-4">
            <APMPanel blocking={result?.blocking_probability} mean_wait={result?.mean_wait} avg_concurrent={avgConcurrent} />
          </div>

          {sensitivity ? (
            <div className="glass-card mb-4">
              <SensitivityChart
                A_values={sensitivity.A_values}
                blocking_sim_mean={sensitivity.blocking_sim_mean}
                blocking_theory={sensitivity.blocking_theory}
                wait_sim_mean={sensitivity.wait_sim_mean}
                wait_theory={sensitivity.wait_theory}
              />
            </div>
          ) : (
            <div className="card mb-4">
              <h3 className="font-semibold text-white">Resultados de la última simulación</h3>
              <div className="mt-2 text-sm text-white/90">Probabilidad de bloqueo: <strong className="text-white">{result?.blocking_probability ?? '-'}</strong></div>
              <div className="mt-1 text-sm text-white/90">Tiempo medio de espera (s): <strong className="text-white">{result?.mean_wait ?? '-'}</strong></div>
            </div>
          )}

          <div className="glass-card">
            <StatsPanel data={result?.wait_times} />
          </div>
        </div>

        {/* Loader overlay */}
        {loading && (
          <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
            <div className="text-center text-white">
              <Spinner />
              <div className="mt-2">Ejecutando simulación — por favor espera...</div>
            </div>
          </div>
        )}
      </main>
    </div>
  )
}

export default App
