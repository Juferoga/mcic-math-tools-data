...
def run_sensitivity(mu: float = 1.0, K: int = 10, sample_size: int = 20000, replicates: int = 3, a_min: float = 0.1, a_max: float = 1.5, steps: int = 20, seed: int = 0):
    ...
    for i, A in enumerate(A_values):
        ...
        print(f"A={A:.3f} (lam={lam:.3f}): blocking sim={blocking_sim_mean[-1]:.4f} theory={blocking_theory[-1]:.4f} | wait sim={wait_sim_mean[-1]:.4f} theory={wait_theory[-1]:.4f}")
    ...
    # blocking
    axes[0].errorbar(A_values, blocking_sim_mean, yerr=blocking_sim_std, fmt='o', label='Simulación (media±std)')
    ...
    # waiting time
    axes[1].errorbar(A_values, wait_sim_mean, yerr=wait_sim_std, fmt='o', label='Simulación (media±std)')
